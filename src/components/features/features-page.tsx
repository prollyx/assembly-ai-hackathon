...
const FeaturesPage: FC<FeaturesPageProps> = ({ projectId }) => {
  ...
  const { activeProject, setActiveFeature } = useProjectContext();
  ...
  return (
    <div>
      <div className="flex justify-between">
        ...
        <button onClick={() => setIsFeatureFormVisible(true)}>
          Create Feature <AddIcon />
        </button>
      </div>
      {isFeatureFormVisible && (
        <NewFeatureForm
          projectId={projectId}
          onClose={() => setIsFeatureFormVisible(false)}
        />
      )}
          ...
          data?.map((feature: Feature) => (
            <ProjectCard
              ...
              onClick={() => {
                ...
                push(
                  `/projects/${projectId}/feature/${feature.id}/requirements`
                );
              }}
              ignoreLink
            />
